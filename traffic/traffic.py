...
def load_data(data_dir):
    ...
    for dir in os.listdir(data_dir):
        if dir == '.DS_Store':
              continue
        for image_file in os.listdir(os.path.join(os.fspath(data_dir), dir)):
            labels.append(dir)
            image = cv2.imread(os.path.join(os.fspath(data_dir),dir,image_file))
...
def get_model():
    ...
    x = tf.keras.layers.Conv2D(1024, 3, activation="relu")(inputs)
    ...
    model = tf.keras.Model(inputs=inputs, outputs=outputs, name="getsrb_small")
